...
def set_user_time_db(user_id: int, time: str) -> bool:
    ...
    cursor.execute(
        "INSERT INTO users_time(user_id, time) VALUES (?, ?)", (user_id, time)
    )
...
def delete_user_time_db(user_id: int, time: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM users_time WHERE user_id = ? AND time = ?", (user_id, time)
    )
    conn.commit()
    conn.close()
...
def set_user_city_db(user_id: int, city: str) -> bool:
    ...
    cursor.execute(
        "INSERT INTO users_city(user_id, city) VALUES (?, ?)", (user_id, city)
    )
...
def delete_user_city_db(user_id: int, city: str):
    ...
    cursor.execute(
        "DELETE FROM users_city WHERE user_id = ? AND city = ?", (user_id, city)
    )
    conn.commit()
